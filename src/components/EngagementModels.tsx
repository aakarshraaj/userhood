            ...
            <div className="max-w-[1440px] mx-auto relative z-10">
                ...
                <div className="grid lg:grid-cols-3 gap-4 md:gap-6">
                    {models.map((model, index) => (
                        ...
                        >
                            {model.featured && (
                                <div className="absolute top-0 left-0 right-0 md:left-auto md:right-0 bg-primary text-black font-mono text-xs uppercase tracking-widest px-3 py-1.5 font-bold text-center md:text-left">
                            ...
                            <div className="flex justify-between items-start gap-3 mb-6 md:mb-8 relative z-10">
                                ...
                                <div className="font-mono text-xs text-white/20 tracking-widest shrink-0">{model.id}</div>
                            </div>

                            <div className="relative z-10 mb-6 md:mb-8 flex-grow min-w-0">
                                <div className="font-mono text-xs text-primary mb-1 md:mb-2 tracking-widest uppercase">{model.subtitle}</div>
                            ...
                            <div className="relative z-10 border-t border-white/10 pt-4 md:pt-6 mb-6 md:mb-8 group-hover:border-primary/30 transition-colors">
                                <div className="flex justify-between items-center gap-2 mb-4 md:mb-6">
                                    <span className="font-mono text-xs text-white/40 uppercase tracking-widest">Est_Timeline</span>
                                    <span className="font-mono text-xs text-white font-bold bg-white/5 px-2 py-1 shrink-0">{model.duration}</span>
                                </div>
                ...
                <div className="mt-10 md:mt-16 flex justify-center px-2">
                    <div className="inline-flex items-center gap-2 md:gap-3 border border-white/10 bg-white/5 px-4 py-2.5 md:px-6 md:py-3 font-mono text-xs md:text-xs text-white/60 tracking-widest uppercase text-center">
                        <div className="w-2 h-2 rounded-full bg-red-500 animate-pulse shrink-0" />
                        <span className="break-words">CURRENT_BANDWIDTH: 1 SLOT REMAINING FOR UPCOMING QUARTER</span>
                    </div>
                </div>
            </div>
        </section>
